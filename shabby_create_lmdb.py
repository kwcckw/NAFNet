...
def prepare_keys(folder_path, suffix='png'):
    ...
    img_path_list = sorted(list(glob(folder_path+"/*.png")))
    keys = [img_path.split('.{}'.format(suffix))[0] for img_path in sorted(img_path_list)]

    return img_path_list, keys

def create_lmdb_for_SHABBY():
    
    folder_types = ['train', 'validate']
    
    for folder_type in folder_types:
        folder_path = './datasets/shabby/train/noisy'
        lmdb_path = './datasets/shabby_lmdb/'+folder_type+'/'+folder_type+'_noisy.lmdb'
    
        img_path_list, keys = prepare_keys(folder_path, 'PNG')
        make_lmdb_from_imgs('', lmdb_path, img_path_list, keys)
    
        folder_path = './datasets/shabby/train/clean'
        lmdb_path = './datasets/shabby_lmdb/'+folder_type+'/'+folder_type+'_clean.lmdb'
